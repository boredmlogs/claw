  ...
  it('returns correct count after inserts', () => {
    db.prepare(
      `INSERT INTO registered_groups (jid, name, folder, trigger_pattern, added_at, requires_trigger)
       VALUES (?, ?, ?, ?, ?, ?)`,
    ).run('123@g.us', 'Group 1', 'group-1', '@Lauren', '2024-01-01T00:00:00.000Z', 1);

    db.prepare(
      `INSERT INTO registered_groups (jid, name, folder, trigger_pattern, added_at, requires_trigger)
       VALUES (?, ?, ?, ?, ?, ?)`,
    ).run('456@g.us', 'Group 2', 'group-2', '@Lauren', '2024-01-01T00:00:00.000Z', 1);
...
describe('credentials detection', () => {
  ...
  it('returns false when no credentials', () => {
    const content = 'ASSISTANT_NAME="Lauren"\nOTHER=foo';
    const hasCredentials = /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY)=/m.test(content);
