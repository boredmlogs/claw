...
function parseArgs(args: string[]): RegisterArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--assistant-name': result.assistantName = args[++i] || 'Lauren'; break;
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  if (parsed.assistantName !== 'Lauren') {
    logger.info({ from: 'Lauren', to: parsed.assistantName }, 'Updating assistant name');
    ...
    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        ...
        content = content.replace(/^# Lauren$/m, `# ${parsed.assistantName}`);
        content = content.replace(/You are Lauren/g, `You are ${parsed.assistantName}`);
