...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Lauren';
...
Messages must start with the trigger pattern (default: `@Lauren`):
- `@Lauren what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @Lauren` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Lauren what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Lauren what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Lauren add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Lauren remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Lauren list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Lauren remember I prefer dark mode` | Add to global memory |
...
```
User: @Lauren remind me every Monday at 9am to review the weekly metrics
...
```
User: @Lauren at 5pm today, send me a summary of today's emails
...
From any group:
- `@Lauren list my scheduled tasks` - View tasks for this group
- `@Lauren pause task [id]` - Pause a task
- `@Lauren resume task [id]` - Resume a paused task
- `@Lauren cancel task [id]` - Delete a task

From main channel:
- `@Lauren list all tasks` - View tasks from all groups
- `@Lauren schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Lauren add group "Name"` in main |
